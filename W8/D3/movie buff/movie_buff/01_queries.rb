...
def harrison_ford
  ...
  #   actors AS H_actors
  # JOIN
  #  castings AS H_castings ON (H_castings.actor_id = H_actors.id)
  # JOIN
  #   movies ON (H_castings.movie_id = movies.id)
    
  # JOIN castings ON castings.movie_id = H_castings.movie_id
  # JOIN actors ON actors.id = castings.actor_id
  # WHERE H_actors.name = 'Harrison Ford' AND H_castings.ord != 1;
  # SQL
